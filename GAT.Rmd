...
```{r importarArch, message=FALSE, warning=FALSE, include=FALSE}
 ...
 { # Create Base Theme
  ...
  ########################
  ### LIQUIDADO TGI
  liquidadoTGI <- read_excel("THI/LIQUIDADO_TGI.xlsx")
  ...
  ESTADO_TGI <- read_excel("THI/ESTADO_TGI.xlsx")
  ...
  ########################
  ### LIQUIDADO OSM
  liquidadoOSM <- read_excel("THI/LIQUIDADO_OSM.xlsx")
  ...
  ### ESTADO DE LO LIQUIDADO TGI
  ESTADO_OSM <- read_excel("THI/ESTADO_OSM.xlsx")
  ...
  ########################
  ### LIQUIDADO THI
  liquidado <- read_excel("THI/LIQUIDADO_THI.xlsx")
  ...
  ## THI por Estado
  ESTADO_THI <- read_excel("THI/ESTADO_THI.xlsx")
  ...
  ## 
  DEUDA_2015 <- read_excel("THI/DEUDA2015_2020.xlsx", "2015")
  DEUDA_2016 <- read_excel("THI/DEUDA2015_2020.xlsx", "2016")
  DEUDA_2017 <- read_excel("THI/DEUDA2015_2020.xlsx", "2017")
  DEUDA_2018 <- read_excel("THI/DEUDA2015_2020.xlsx", "2018")
  DEUDA_2019 <- read_excel("THI/DEUDA2015_2020.xlsx", "2019")
  # DEUDA_2020 <- read_excel("THI/DEUDA2015_2020.xlsx", "2020")

  COMERCIOS <- read_excel("THI/COMERCIOS_ACTIVOS.xlsx")
  contibuciones <-read_csv("THI/contribuciones.csv")

  contibuciones <- contibuciones %>%   filter(cuit %in% COMERCIOS$CUIT)%>% left_join(COMERCIOS, by=c("cuit"="CUIT"))
  
  salarios <- read_csv("THI/salarios.csv")
  
  salarios<- salarios %>% filter(cuit %in% COMERCIOS$CUIT)%>%left_join(COMERCIOS, by=c("cuit"="CUIT"))
```
...
### Comercios ATP
Listado de empresas beneficiarias de la postergación o reducción de las contribuciones patronales. 
En la ciudad se registran actualmente `r length(unique(COMERCIOS$CUIT))` comercios con CUIT en el sistema de gestión tributaria. 

De los mismos `r length(unique(contibuciones$razon_social)) ` han accedido al beneficio del ATP, es decir, el *`r round(length(unique(contibuciones$razon_social))/length(unique(COMERCIOS$CUIT))*100, 1)` %* del total registrado.  



En el siguiente listado se encuentran todos los comercios de la ciudad que recibieron el ATP. 
```{r activos, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
 
 atp <- contibuciones %>% 
    select(sector,  razon_social, nombre=`NOMBRE DE FANTASIA`, cuit, 
           salario_devengado_mes, ronda_atp, domicilio = `DOMICILIO COMERCIAL`) %>% unique() %>% 
  arrange(sector,razon_social, salario_devengado_mes )

  datatable(atp, class = 'cell-border stripe',rownames = FALSE,
            colnames = c("Sector", "Razon Social", "Negocio",  "CUIT", "Mes Dev.", "Ronda ATP", "Domicilio"),
            options = list(pageLength = 10, autoWidth = TRUE))

```

Fuente: Extraído del CEP-XXI en base a información de AFIP y ANSES. Listado provisorio al 21/7, sujeto a revisión.

<!-- ### Salarios  -->
<!-- Listado que corresponde a empleadores con al menos un empleado con salario complementario abonado en cuenta bancaria (por salarios devengados en abril). Incluye a quienes posteriormente devolvieron total o parcialmente el monto girado. Listado provisorio al 21/7, sujeto a revisión. Fuente: CEP-XXI en base a información de AFIP y ANSES -->

<!-- ```{r salarios} -->
<!--   table(salarios$cuit) -->
<!--   # salarios %>%  filter(cuit=="30715857797") -->
